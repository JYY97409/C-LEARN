...
//这里还是存在小巧思的，就是在查找的时候是根据是不是empty来判断的，不是delete，这还是有智慧的
...
inline unsigned long __stl_next_prime(unsigned long n)
{
	...
	//之所以要使用素数表，是因为使用2的n次幂每次只会有几个位的数据参加
...
//但是开放定址法还是没有从根本上解决问题，其实还是占用其他数据的位置，其实是没什么效果的
...
namespace hash_bucket
{
	...
	public:
		...
		//ptr和ref都是模版的妙用，这是因为原来是要写两个迭代器的
		Ptr operator->()
		{
			return &(_node->_data);
		}
	...
	private:
		Node* _node;
		HashTable* _ht;
		//迭代器之所以要包含HashTable，是因为迭代器是需要++的，还是需要hash_table的
